export function iddfs(grid, startNode, endNode) {
    ...
    function dls(node, limit, visitedInThisIteration, path, currentMaxDepth) {
        ...
        // Optional: directions.sort((a, b) => distance(neighborA, end) - distance(neighborB, end))

        for (const [dr, dc] of directions) {
            if (dls({ row: row + dr, col: col + dc }, limit - 1, visitedInThisIteration, path, currentMaxDepth)) {
                return true;
            }
        }
